...
export function ThemeToggle({
  ...
  if (!mounted) {
    return (
      <button
        ...
        className={cn(
          compact
            ? "inline-flex items-center justify-center opacity-50"
            : cn(uiBtnGhost, "shrink-0 px-2 py-1.5 text-xs opacity-50"),
          className,
        )}
      >
        {compact ? "…" : "Tema…"}
  ...
  return (
    <button
      ...
      className={cn(
        ...
        compact
          ...
          : cn(uiBtnGhost, "shrink-0 gap-1.5 px-2 py-1.5 text-xs"),
        className,
      )}
    ...
    >
      {icon}
      {compact ? null : isLight ? "Scuro" : "Chiaro"}
    </button>
  );
}
