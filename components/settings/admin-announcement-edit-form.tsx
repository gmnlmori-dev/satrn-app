...
import { adminUpdateAnnouncement } from "@/lib/actions/admin-update-announcement";
import {
  AdminAnnouncementFormFields,
  announcementFormStateFromAnnouncement,
...
export function AdminAnnouncementEditForm({
  announcement,
  teams,
  profiles,
  onSuccess,
  ...
  const [state, setState] = useState<AnnouncementFormState>(() =>
    announcementFormStateFromAnnouncement(announcement, teams),
  );
      ...
      <div className="flex min-h-0 min-w-0 flex-1 flex-col pr-4 sm:pr-5">
        <div className="min-h-0 flex-1 divide-y divide-line-default overflow-y-auto pb-4 pr-3.5 sm:pr-5">
          <AdminAnnouncementFormFields
            ...
            profiles={profiles}
            disabled={pending}
            idPrefix={`edit-announcement-${announcement.id}`}
          />
        </div>
